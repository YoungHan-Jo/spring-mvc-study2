...
//    @PostMapping("/login")
    ...
    public String loginV3(@Valid @ModelAttribute LoginForm form, BindingResult bindingResult,
        ...
        // 세션에 로그인 회원 정보 보관 , 실제서버에는 메모리 용량 때문에, 세션에는 최소한의 PK 값만 저장해서 불러와야함.
...
//    @PostMapping("/logout")
    ...
    public String logoutV3(HttpServletRequest request) {
        HttpSession session = request.getSession(false); // false로 하면 세션객체를 생성하지않고 있는 세션을 가져온다. 없으면 null 반환
        if (session != null) {
